...
namespace PresentationApp.Pharmacy
{
    public partial class frmPharmacy_Custom : System.Web.UI.Page
    {
        ...
        private void LoadPredefinedLabel_Field(int FeatureID, int PatientID)
        {
                ...
                Session["Age"] = ((DataSet)ViewState["MasterData"]).Tables[6].Rows[0]["Age"];
                if (!IsPostBack)
                {
                    if (VisitID > 0)
                    {
        ...
        private Boolean SetBusinessrule(string FieldID, string FieldLabel)
        {
            ...
            if ((FieldID == "317") && Session["SCMModule"] != null)
            {
                return true;
            }
        ...
        void DispenseGrid_ItemCreated(object sender, GridItemEventArgs e)
        {
        ...
        void gridInstruction_ItemDataBound(object sender, GridItemEventArgs e)
        {
        }
        void gridInstruction_ItemCreated(object sender, GridItemEventArgs e)
        {
            GridDataItem parentItem = ((sender as RadGrid).NamingContainer as GridNestedViewItem).ParentItem as GridDataItem;
        ...
        void gridRefill_ItemCreated(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
        ...
        void DispenseGrid_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
        ...
        void ddBatch_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            DataView dv = new DataView(((DataSet)ViewState["MasterData"]).Tables[32]);
            ...
            foreach (GridNestedViewItem nestedView in rgdrugmain.MasterTableView.GetItems(GridItemType.NestedView))
            {
                ...
                foreach (GridDataItem item in DispenseGrid.Items)
                {
                    ...
                    //RadNumericTextBox txtqtydispense = (RadNumericTextBox)item.FindControl("txtQtyDispensed");
        ...
        void OrderGrid_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
        ...
        void OrderGrid_ItemCreated(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                BindFunctions theBind = new BindFunctions();
        ...
        private StringBuilder SaveCustomFormData(int PatientID, DataSet DS, int DQSaveChk)
        {
            ...
            if (Convert.ToInt32(Session["PatientVisitId"]) == 0)
            {
                ...
                SbInsert.Append("values(" + PatientID + ", " + Session["AppLocationId"] + ", '" + txtpharmOrderedbyDate.Value + "', " + TempDS.Tables[0].Rows[0]["VisitTypeID"].ToString() + ", " + DQSaveChk + ", " + Session["AppUserId"] + ", " + ddlPharmOrderedbyName.SelectedValue+ ", getdate())");
                //              
        ...
        private Boolean FieldValidation(DataTable theDrugDT)
        {
            ...
            else
            {
                ...
                {
                    for (int y = 0; y < theDrugDT.Columns.Count; y++)
                    {
                        if (VisitID == 0)
                        {

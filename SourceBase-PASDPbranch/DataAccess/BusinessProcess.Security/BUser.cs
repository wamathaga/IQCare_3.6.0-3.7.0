    ...
    public class BUser : ProcessBase,IUser 
    {
        ...
        public int UpdateAppointmentStatus(string Currentdate,int locationid )
        {
            lock (this)
            {
                ...
                ClsUtility.AddParameters("@locationid", SqlDbType.Int, locationid.ToString());
                return (int)LoginManager.ReturnObject(ClsUtility.theParams, "pr_Scheduler_UpdateAppointmentStatusMissedAndMet_Constella", ClsUtility.ObjectEnum.ExecuteNonQuery);
            }
        }
      #endregion
    }

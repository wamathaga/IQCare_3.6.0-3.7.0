    ...
    public class BFormBuilder : ProcessBase, IFormBuilder
    {
        public DataTable SystemDate()
        {
            lock (this)
            {
                ...
                DataTable theCurrentDt = (DataTable)DateManager.ReturnObject(ClsUtility.theParams, theSQL, ClsUtility.ObjectEnum.DataTable);
                return theCurrentDt;
            }
        }
        public DataSet GetFormDetail(Int32 iFeatureId)
        {
            lock (this)
            {
                ...
                if (iFeatureId == 0)
                {
                    dsRes = (DataSet)CustomField.ReturnObject(ClsUtility.theParams, "Pr_FormBuilder_FetchPharmacyFormStaticFieldDetail_Futures", ClsUtility.ObjectEnum.DataSet);
                }
                else
                {
             ...
             //{
                 ...
                 if (iFeatureId != 0)
                 {
                     if (dtbusinessRules.Rows.Count == 0)
                     {
                         ...
                         theRowAffected = (int)FormDetail.ReturnObject(ClsUtility.theParams, "pr_FormBuilder_DeleteFormBusinessRules", ClsUtility.ObjectEnum.ExecuteNonQuery);
                     }
                     for (int i = 0; i < dtbusinessRules.Rows.Count; i++)
                     {
                         ...
                         theRowAffected = (int)FormDetail.ReturnObject(ClsUtility.theParams, "pr_FormBuilder_SaveUpdateFormBusinessRules", ClsUtility.ObjectEnum.ExecuteNonQuery);

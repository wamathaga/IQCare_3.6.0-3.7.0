    ...
    public class BUserRole : ProcessBase, IUserRole
    {
            ...
            try
            {
                ...
                if (GroupId != 0)
                {
                    for (int i = 0; i < theDS.Tables[0].Rows.Count; i++)
                    {
                        ...
                        int RowsAffected = (int)UserGroupManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_SaveFacilityServiceUserGroupFunction_Detail", ClsUtility.ObjectEnum.ExecuteNonQuery);
                        if (RowsAffected == 0)
                        {
        ...
        public DataSet GetSavedData()
        {
            lock (this)
            {
                ClsUtility.Init_Hashtable();
                ClsObject UserRoleManager = new ClsObject();
                return (DataSet)UserRoleManager.ReturnObject(ClsUtility.theParams, "pr_GetFacilityServiceData", ClsUtility.ObjectEnum.DataSet);
            }

        ...
        private void Init_ManageCareEnded()
        {
            ...
            else
            {
                ...
                if (GblIQCare.strMainGrdFldName == "Exit Reason" && GblIQCare.SystemId == 3)
                {
                    ...
                    dsConditional.Tables.Add(theDT);
                }
                ...
                else if (GblIQCare.strMainGrdFldName == "Exit Reason" && GblIQCare.SystemId == 1)
                {
                    ...
                    BindFunctions theBindManager = new BindFunctions();
                    DataView theDV = new DataView(XMLDS.Tables["Mst_Decode"]);
                    theDV.RowFilter = "((DeleteFlag =0 or DeleteFlag is null) and CodeId=23 and SystemId=0)";
                    theDV.Sort = "Name ASC";
                    DataTable theDT = theDV.ToTable();
                    dsConditional.Tables.Add(theDT);
                }
        ...
        void theCombo_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            if (theGrdCombo.Focused == true && theGrdCombo.SelectedValue != null)
            {
                if (theGrdCombo.DataSource != null && theGrdCombo.SelectedValue.GetType().ToString() != "System.Data.DataRowView")
                {
                    ...
                    if (theDV.Count > 0)
                    {
                        if (dgwConditionalField.CurrentRow.Cells[0].Value != theDV[0]["ID"].ToString())
                        {

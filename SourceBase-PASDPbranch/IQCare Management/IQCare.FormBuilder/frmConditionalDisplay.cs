            ...
            try
            {
                ...
                if (dsFieldDetails.Tables[2].Rows.Count > 0 && GblIQCare.iManageCareEnded==0)
                {
                    if (!CheckFormVersion(theGrdSource.Copy(), theExistingFields))
                    {
                        ...
                        DialogResult msgconfirm = IQCareWindowMsgBox.ShowWindow(string.Format("There are some change(s) you had made on this Field and this field is used on" + dsFieldDetails.Tables[2].Rows.Count + " Form(s).{0} Do you want change the Form Version?", Environment.NewLine), "?", "", this);

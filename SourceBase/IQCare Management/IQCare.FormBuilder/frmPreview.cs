    ...
    public partial class frmPreview : Form
    {
        ...
        private void frmPreview_Load(object sender, EventArgs e)
        {
            ...
            if (GblIQCare.iFormBuilderId == 126)
            {
                ...
                GblIQCare.iFormBuilderType = "Registration";
                panel1.Visible = false;
                panel3.Visible = true;
                this.panel3.Location = new System.Drawing.Point(0, 0);
            }
            LoadPredefinedLabel_Field(GblIQCare.iFormBuilderId, 0, GblIQCare.iFormBuilderType);
            //set css begin
        ...
        private void LoadFieldTypeControl(string ControlID, Panel PnlAddControl, string Column, string ID, string CodeID, string Label, string Table, string BindTable)
        {
            string strNewLine;
            int lblength = 0;
            ...
            else if (ControlID == "4") /// Dropdown
            {
                ...
                //DataSet theDS = CustomFormMgr.GetPmtctDecodeTable(CodeID, BindTable);
                //DataTable theDT = theDS.Tables[0];
                DataTable theDT = getselectMultiselectvalues(BindTable,CodeID);
                if (theDT != null)
                {
        ...
        private void LoadPredefinedLabel_Field(int FeatureID, int PatientID, string FormType)
        {
            ...
            if (FormType == "Pharmacy")
            {
                ...
                theDS = IPatientCustomFormMgr.GetFieldName_and_LabelPharmacy(FeatureID, PatientID);
            }
            else
            {
                ICustomForm CustomFormMgr = (ICustomForm)ObjectFactory.CreateInstance(ObjFactoryParameter);
                theDS = CustomFormMgr.GetFieldName_and_Label(GblIQCare.iFormBuilderId, 0);
            }
                ...
                foreach (DataRow DRLnkTable in theDS.Tables[1].Rows)
                {
                    if (DT.Rows[i]["SectionID"].ToString() == DRLnkTable["SectionID"].ToString())
                    {
                        ...
                        if (td <= Numtds)
                        {
                                ...
                                LoadFieldTypeControl(DRLnkTable["Controlid"].ToString(), pnlControlCol, DRLnkTable["FieldName"].ToString(), DRLnkTable["FieldID"].ToString(), DRLnkTable["CodeID"].ToString(), DRLnkTable["FieldLabel"].ToString(), DRLnkTable["PDFTableName"].ToString(), DRLnkTable["BindSource"].ToString());

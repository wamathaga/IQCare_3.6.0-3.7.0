...
namespace IQCare.SCM
{
    public partial class frmItemMaster : Form
    {
        ...
        private void CalucateDosesDispenseUnit()
        {
            Int32 DispUnitQty = 0;
            decimal lblVolUnits = 0;
            if (txtDispensingUnitQuantity.Text == "")
                DispUnitQty = 0;
            else
                DispUnitQty = Convert.ToInt32(txtDispensingUnitQuantity.Text);

            if (txtPerlblVolUnits.Text == "")
                lblVolUnits = 0;
            else
                lblVolUnits = Convert.ToDecimal(txtPerlblVolUnits.Text);
            if (DispUnitQty == 0 || lblVolUnits == 0)
                txtDispensingUnit.Text = "0";
            else
                txtDispensingUnit.Text = Math.Round(DispUnitQty / lblVolUnits).ToString();
        }
        ...
        private Hashtable BuildHashTable()
        {
            ...
            theHash.Add("ItemInstructions", txtInstructions.Text);
            theHash.Add("DispenseUnitQty", txtDispensingUnitQuantity.Text);
            theHash.Add("VolumeUnit", cmbVolumeUnit.SelectedValue.ToString());
            theHash.Add("MedicationAmt", txtMedicationAmt.Text);
            theHash.Add("PerlblVolUnits", txtPerlblVolUnits.Text);
            theHash.Add("DispesingUnit", txtDispensingUnit.Text);
        ...
        private void txtPerlblVolUnits_KeyUp(object sender, KeyEventArgs e)
        {
            CalucateDosesDispenseUnit();
        }

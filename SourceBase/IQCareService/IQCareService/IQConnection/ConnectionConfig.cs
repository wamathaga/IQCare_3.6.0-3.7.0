...
namespace IQConnection
{
    public partial class ConnectionConfig : Form
    {
        ...
        private int CheckIQToolsData()
        {
            ...
            {
                MessageBox.Show("Invalid IQtools Password. Please try again.");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string theConString = string.Empty;
            #region IQCare
            if (CheckData() == 1)
            {

                #region "TempConfigSettings"
                theConString = objUtil.Encrypt(string.Format("data source = {0};uid = {1};pwd = {2};initial catalog = {3}", txtServernm.Text.Trim(), txtUsernm.Text.Trim(), txtPassword.Text.Trim(), txtDBnm.Text.Trim()));
                ...
                if (nodeIQCare != null)
                {
                    nodeIQCare.Attributes["value"].Value = theConString;
                }
                else
                {
                    var node = doc.CreateElement("add");
                    node.SetAttribute("key", "ConnectionString");
                    node.SetAttribute("value", theConString);
                ...
                {
                    config.AppSettings.Settings.Add("ConnectionString", theConString);
            ...
            theConString = objUtil.Encrypt(string.Format("data source = localhost;database=iqcare;uid = {0};pwd = {1};", txtMySQLUID.Text.Trim(), txtMySQLPWD.Text.Trim()));

            strIQCareDB = databaseName;

            XmlNode nodeMySQL = doc.SelectSingleNode("//appSettings/add[@key='MySQLConnectionString']");
            if (nodeMySQL != null)
            {
                nodeMySQL.Attributes["value"].Value = theConString;
            }
            else
            {
                var node = doc.CreateElement("add");
                node.SetAttribute("key", "MySQLConnectionString");
                node.SetAttribute("value", theConString);
                doc.SelectSingleNode("//appSettings").AppendChild(node);
            }
            if (ConfigurationManager.AppSettings["MySQLConnectionString"] != null)
            {
                ConfigurationManager.AppSettings.Set("MySQLConnectionString", theConString);
            }
            else
            {
                config.AppSettings.Settings.Add("MySQLConnectionString", theConString);
                config.Save(ConfigurationSaveMode.Modified);
            }
            #endregion
            ...
            if (ConfigIQtools.Checked = true)
            {
                if (CheckIQToolsData() == 1)
                {
                    ...
                    if (nodeIQTools != null)
                    {
                        nodeIQTools.Attributes["value"].Value = theConString;
                    }
                    else
                    {
                        var node = doc.CreateElement("add");
                    ...
                    {
                        ConfigurationManager.AppSettings.Set("IQToolsConnectionString", theConString);
                    }
                    else
                    {
                        config.AppSettings.Settings.Add("IQToolsConnectionString", theConString);
                        config.Save(ConfigurationSaveMode.Modified);
                    }

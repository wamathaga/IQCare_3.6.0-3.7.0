...
function buildWeeklyCalendar(WeekStart, Position) {
    ...
    if (typeof this.href === "undefined") {
        CurrentURL = document.location.toString().toUpperCase();
    }
    else {
        CurrentURL = this.href.toString().toUpperCase();
    }
    var pathArray = location.pathname.split('/');
    if (pathArray.length > 3 && (CurrentURL.indexOf('IQCARE') > -1) == true) {
        pathArray = '../';
    }
    else if (pathArray.length > 2 && (CurrentURL.indexOf('IQCARE') > -1) == true) {
        pathArray = './';
    }
    else if (pathArray.length > 2 && (CurrentURL.indexOf('IQCARE') > -1) == false) {
        pathArray = '../';
    }
    else {
        pathArray = './';
    ...
    //Jayant 04-03-2008-Begin
    if (WeekStart == 0 && Position == 0) {
        //  document.write('<td align="center" onClick="w_hiddenCalendar()"><img src="/' + pathArray + '../images/close.gif" title="' + close_title + '" border="0"></td>'); }
        document.write('<td align="center" onClick="w_hiddenCalendar()"><img src="' + pathArray + 'images/close.gif" title="' + close_title + '" border="0"></td>');
    }

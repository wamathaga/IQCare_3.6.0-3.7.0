    ...
    public partial class PatientDeposits : System.Web.UI.UserControl
    {
        ...
        protected void btnNewDeposit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (amount > 0.0M)
                {
                    ...
                    string tranRef = dt.Rows[0]["TransactionReference"].ToString();
                    ...
                    string theUrl = string.Format("{0}?ReceiptTrxCode={1}&RePrint=false&TranXORType=9", this.PrintReceiptURL, transactionID);
                    string urlParam = String.Format("{0}('{1}')",this.PrintReceiptJSMethod,theUrl);

                    this.btnOkAction.OnClientClick = string.Format("javascript:{0};return true;",urlParam );
                   ...
                   // ScriptManager.RegisterStartupScript(Page, Page.GetType(),"PRINTRECEIPT",urlParam,true);
                    ...
                    this.NotifyAction("Successful. Deposit received of " + textAmount.Text.Trim(), "Making Deposit", false);
        ...
        protected void btnRefundYes_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (amount > 0.0M)
                {
                    this.BillingManager.ExecuteDepositTransaction(this.PatientID, this.LocationID, this.UserID, amount, DepositTransactionType.ReturnDeposit);
                    this.NotifyAction("Successful. Deposit refunded amounting to " + amount.ToString(), "Refunding Deposit", false);
                }
                else
                {

    ...
    <xsl:template match="/" >
        <head>
            ...
            <script LANGUAGE="Javascript">
                ...
                function PrintMe(){if (confirm("Print this price list?")){   window.print();     }  
            </script>
        </head>
            ...
            <center>
                ...
                <form id="frmPrint" name="form1">
                    <p align="center" class="btn">
                        <input type="button" name="cmdPrint" class="btn" style="width: 75px" value="Print" onclick="window.print(); return false;"></input>&nbsp;
				...
				<td style="white-space:nowrap">
					...
					<xsl:choose>
						<xsl:when test="$ItemTypeName = 'Pharmaceuticals'">
						 per 
						<xsl:choose>
							<xsl:when test="PricePerType = 'true'">
								Dose
							</xsl:when>
							<xsl:otherwise>
								Item

    ...
    public class BMigration: ProcessBase,IMigration 
    {
        ...
        public int MigrateData(string cmd, int Location, string DBName, int CommandId)
        {
            ...
            try
            {
                ...
                int RowsEffected = (int)objMigration.ReturnObject(ClsUtility.theParams, cmd, ClsDBUtility.ObjectEnum.ExecuteNonQuery);

                string thecmd = "update migration set complete = 1 where id = " + CommandId;
                ClsUtility.Init_Hashtable();
                int rowseffected = (int)objMigration.ReturnObject(ClsUtility.theParams, thecmd, ClsDBUtility.ObjectEnum.ExecuteNonQuery);
                //////theTran.Commit();
                //////theCon.Close();
                DataMgr.CommitTransaction(this.Transaction);
                DataMgr.ReleaseConnection(this.Connection);
        ...
        public DataSet GetProcedures(string Constr,string DBName)
        {
            lock (this)
            {
                try
                {
                    ...
                    //////ClsUtility.Init_Hashtable();
                    //////ClsUtility.AddParameters("@dbname", SqlDbType.VarChar, DBName);
                    //////return (DataSet)theObject.ReturnObject(Constr, ClsUtility.theParams, "pr_GetProcedureList", ClsDBUtility.ObjectEnum.DataSet);
                    ...
                    ClsUtility.Init_Hashtable();
                    ClsUtility.AddParameters("@dbname", SqlDbType.VarChar, DBName);
                    return (DataSet)objProcList.ReturnObject(ClsUtility.theParams, "pr_GetProcedureList", ClsDBUtility.ObjectEnum.DataSet);
                }
                catch
                {

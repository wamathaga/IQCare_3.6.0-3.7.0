...
namespace BusinessProcess.Clinical
{
    public class BHivCareARTEncounter : ProcessBase, IHivCareARTEncounter
    {
        ...
        #region "HIVCare and ART Encounter"
         public DataSet GetExistHIVArtCareEncounterbydate(int PatientID, string VisitdByDate ,int locationID)
        {
            lock (this)
            {
                ...
                return (DataSet)VisitManager.ReturnObject(ClsUtility.theParams, "pr_HIVArtCareEncounter_DateValidate_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public DataSet GetHIVCareARTPatientFormData(int patientID, int locationID)
        {
            lock (this)
            {
                ...
                ClsObject VisitManager = new ClsObject();
                return (DataSet)VisitManager.ReturnObject(ClsUtility.theParams, "pr_Clinical_GetHIVCareARTPatientFormData", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        public DataSet GetHIVCareARTPatientVisitInfo(int patientID, int locationID, int visitID)
        {
            lock (this)
            {
                ...
                ClsUtility.AddParameters("@locationID", SqlDbType.Int, locationID.ToString());
                ClsObject VisitManager = new ClsObject();
                return (DataSet)VisitManager.ReturnObject(ClsUtility.theParams, "pr_Clinical_GetHIVCareARTPatientVisitInfo", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        public DataSet SaveUpdateHIVCareARTPatientVisit(Hashtable hashTable, DataSet dataSet, bool isUpdate, DataTable theCustomDataDT)
        {
            try
            {
                ...
                {
                    // DataSet tempDataSet;
                    theDS = (DataSet)VisitManager.ReturnObject(ClsUtility.theParams, "pr_Clinical_SaveHIVCareARTPatientVisit", ClsDBUtility.ObjectEnum.DataSet);
                ...
                else
                {
                    ...
                    theDS = (DataSet)VisitManager.ReturnObject(ClsUtility.theParams, "pr_Clinical_UpdateHIVCareARTPatientVisit", ClsDBUtility.ObjectEnum.DataSet);
                    //Family Planning Methods
                    //for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
                    //{
                    //    ClsUtility.Init_Hashtable();
                    ...
                    //    ClsUtility.AddParameters("@nutritionalSupport", SqlDbType.Int, hashTable["nutritionalSupport"].ToString());
                    //    ClsUtility.AddParameters("@createDate", SqlDbType.DateTime, hashTable["createDate"].ToString());
                    //    int temp = (int)VisitManager.ReturnObject(ClsUtility.theParams, "pr_Clinical_UpdateHIVCareFamilyPlanning", ClsDBUtility.ObjectEnum.ExecuteNonQuery);
                    //}

    ...
    public class BCodeDecode : ProcessBase,ICodeDecode 
    {
        ...
        public DataSet GetCodes()
        {
            lock (this)
            {
                ...
                return (DataSet)CodeManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectCode_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public DataSet GetDecCodes()
        {
            lock (this)
            {
                ...
                return (DataSet)DeCodeManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectDeCode_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public int SaveNewCode(string Name, int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)CodeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_AddCode_Constella", ClsDBUtility.ObjectEnum.DataRow);
        ...
        public int UpdateCode(int CodeId,string Name, int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)CodeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateCode_Constella", ClsDBUtility.ObjectEnum.DataRow);
                if (Convert.ToInt32(theDR[0]) == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
        ...
        public int SaveNewDeCode(string Name, int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)DeCodeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_AddDeCode_Constella", ClsDBUtility.ObjectEnum.DataRow);
                if (Convert.ToInt32(theDR[0]) == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
        ...
        public int UpdateDeCode(string Name, int ID,int UserId)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)DeCodeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateDeCode_Constella", ClsDBUtility.ObjectEnum.DataRow);
                if (Convert.ToInt32(theDR[0]) == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
                    theBL.DataElements["MessageText"] = "Error in Saving DeCode record. Try Again..";

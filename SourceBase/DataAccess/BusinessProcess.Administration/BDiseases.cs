    ...
    public class BDiseases : ProcessBase,IDiseases 
    {
        ...
        public DataSet GetDiseases()
        {
            lock (this)
            {
                ...
                return (DataSet)DiseaseManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectDisease_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        public DataSet DeleteDisease(int diseaseid)
        {
            lock (this)
            {
                ...
                return (DataSet)DiseaseManager.ReturnObject(ClsUtility.theParams, "pr_Admin_DeleteDisease_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        public DataSet GetDiseasesByID(int diseaseid)
        {
            lock (this)
            {
                ...
                return (DataSet)DiseaseManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectDiseasesByID_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public int SaveNewDisease(string DiseaseName, int UserID,int Sequence)
        {
            ...
            try
            {
             ...
             //   ClsUtility.AddParameters("@DeleteFlag", SqlDbType.Int, DeleteFlag.ToString());
                ...
                DataRow theDR;
                int RowsAffected = (Int32)DiseaseManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_AddDisease_Constella", ClsDBUtility.ObjectEnum.ExecuteNonQuery);
                if (RowsAffected == 0)
                {
        ...
        public int UpdateDisease(int Disease_Pk,string DiseaseName, int UserID,int DeleteFlag,int Sequence)
        {
            ...
            try
            {
                ...
                DataRow theDR;
                int RowsAffected  = (Int32)DiseaseManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateDisease_Constella", ClsDBUtility.ObjectEnum.ExecuteNonQuery);
                if (RowsAffected == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
                    theBL.DataElements["MessageText"] = "Error in Saving Disease record. Try Again..";

    ...
    public class BControls : ProcessBase, IControl
    {
        ...
        public DataSet GetControl()
        {
            lock (this)
            {
                ...
                return (DataSet)ControlManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectControl_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public int SaveNewControl(string Name, string sDataType,int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)ControlManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_AddControl_Constella", ClsDBUtility.ObjectEnum.DataRow);
        ...
        public int UpdateControl(int ControlId,string Name, string sDataType, int UserID)
        {
            ...
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)ControlManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateControl_Constella", ClsDBUtility.ObjectEnum.DataRow);
                if (Convert.ToInt32(theDR[0]) == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
                    theBL.DataElements["MessageText"] = "Error in Saving Control record. Try Again..";

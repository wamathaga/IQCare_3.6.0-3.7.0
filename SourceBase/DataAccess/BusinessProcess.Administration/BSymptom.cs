    ...
    public class BSymptom : ProcessBase,ISymptom
    {
        ...
        public DataSet GetSymptom()
        {
            lock (this)
            {
                ...
                return (DataSet)SymptomManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectSymptom_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public int SaveNewSymptom(int SymptomCategoryID, string SymptomName, int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)SymptomManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_InsertSymptom_Constella", ClsDBUtility.ObjectEnum.DataRow);
        ...
        public int UpdateSymptom(int SymptomID,int SymptomCategoryID, string SymptomName, int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)SymptomManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateSymptom_Constella", ClsDBUtility.ObjectEnum.DataRow);
                if (Convert.ToInt32(theDR[0]) == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
                    theBL.DataElements["MessageText"] = "Error in Saving Symptom record. Try Again..";

    ...
    public class BEmployeeMst : ProcessBase, IEmployeeMst
    {
        ...
        public DataSet GetEmployeeForID(int EmployeeID)
        {
            lock (this)
            {
                ...
                return (DataSet)EmployeeManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectEmployeeForID_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        public DataSet GetEmployee()
        {
            lock (this)
            {
                ClsUtility.Init_Hashtable();
                ClsObject EmployeeManager = new ClsObject();
                return (DataSet)EmployeeManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectEmployee_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public DataTable SaveNewEmployee(string FirstName, string LastName, int DesignationID,int EmployeeID,int DeleteFlag,int UserID)
        {
            try
            {
                ...
                if (EmployeeID == 0)
                {
                    theAffectedDT = (DataTable)EmployeeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_InsertEmployee_Constella", ClsDBUtility.ObjectEnum.DataTable);
                ...
                else
                {
                    ...
                    ClsUtility.AddParameters("@EmployeeID", SqlDbType.Int, EmployeeID.ToString());
                    theAffectedDT = (DataTable)EmployeeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateEmployee_Constella", ClsDBUtility.ObjectEnum.DataTable);
      ...
      //          DataRow theDR;
      //          int RowsAffected = (Int32)EmployeeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateEmployee_Constella", ClsDBUtility.ObjectEnum.ExecuteNonQuery);
      //          if (RowsAffected == 0)
      //          {
      //              MsgBuilder theBL = new MsgBuilder();
      //              theBL.DataElements["MessageText"] = "Error in Saving Employee record. Try Again..";

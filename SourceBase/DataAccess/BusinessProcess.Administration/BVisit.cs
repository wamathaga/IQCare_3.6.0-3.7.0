    ...
    public class BVisit : ProcessBase,IVisit 
    {
        ...
        public DataSet GetVisitTypeByID(int visitid)
        {
            lock (this)
            {
                ...
                return (DataSet)VisitTypeManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectVisitTypesByID_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        public DataSet GetVisitType()
        {
            lock (this)
            {
                ClsUtility.Init_Hashtable();
                ClsObject VisitTypeManager = new ClsObject();
                return (DataSet)VisitTypeManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectVisitTypes_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        public DataSet DeleteVisitType(int visittypeid)
        {
            lock (this)
            {
                ...
                return (DataSet)VisitTypeManager.ReturnObject(ClsUtility.theParams, "pr_Admin_DeleteVisitType_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        public int SaveNewVisitType( string VisitName, int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)VisitTypeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_AddVisitType_Constella", ClsDBUtility.ObjectEnum.DataRow);
        ...
        public int UpdateVisitType(int VisitTypeID, string VisitName, int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)VisitTypeManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateVisitType_Constella", ClsDBUtility.ObjectEnum.DataRow);
                if (Convert.ToInt32(theDR[0]) == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
                    theBL.DataElements["MessageText"] = "Error in Saving Visit type record. Try Again..";

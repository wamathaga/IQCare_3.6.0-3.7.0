    ...
    public class BSection : ProcessBase
    {
        ...
        public DataSet GetSection()
        {
            lock (this)
            {
                ...
                return (DataSet)SectionManager.ReturnObject(ClsUtility.theParams, "pr_Admin_SelectSection_Constella", ClsDBUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public int SaveNewSection(string SectionName,  int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)SectionManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_InsertSection_Constella", ClsDBUtility.ObjectEnum.DataRow);
        ...
        public int UpdateSection(int SectionId,string SectionName, int UserID)
        {
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)SectionManager.ReturnObject(ClsUtility.theParams, "Pr_Admin_UpdateSection_Constella", ClsDBUtility.ObjectEnum.DataRow);
                if (Convert.ToInt32(theDR[0]) == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
                    theBL.DataElements["MessageText"] = "Error in Saving Section record. Try Again..";

    ...
    public class IQToolsQueries : ProcessBase, IReportIQQuery
    {
        ...
        DataTable IReportIQQuery.ExecQuery(DataAccess.Report.Query query)
        {
            try
            {
                ...
                dt = (DataTable)ReportManager.ReturnIQToolsObject(ClsUtility.theParams, query.Defination, ClsDBUtility.ObjectEnum.DataTable, ConnectionMode.REPORT);
        ...
        public DataTable ExecQuery(int queryID, DateTime dateFrom, DateTime dateTo, int cd4CutOff = 350)
        {
            try
            {
                ClsObject ReportManager = new ClsObject();
                ClsUtility.Init_Hashtable();
                string queryStatement = @"Select	qryID Query_ID, qryName Query_Name, qryDefinition Defination,qryDescription Description From dbo.aa_Queries Where qryID = @Query_ID; ";
                ...
                DataRow row = (DataRow)ReportManager.ReturnIQToolsObject(ClsUtility.theParams, queryStatement, ClsDBUtility.ObjectEnum.DataRow, ConnectionMode.REPORT);
        ...
        public string GetFullQueryDocument(string categoryName = "")
        {
            try
            {
                ...
                string queryStatement = @"Select	B.sbCategory SubCategory,
		                                Category,B.catID CategoryID,B.sbCatID SubCategoryID,Q.qryID Query_ID,
		                                Q.qryName Query_Name,Q.qryDescription Description From dbo.aa_Category A
                                        Inner Join dbo.aa_sbCategory B On A.CatID = B.CatID
                                        Inner Join dbo.aa_Queries Q On Q.qryID = B.QryID
                                        Where (B.DeleteFlag = 0 Or B.DeleteFlag Is Null) And (Q.DeleteFlag = 0 Or Q.DeleteFlag Is Null)
                                        And Case When @Category  Is Null Or A.Category = @Category  Then 1 
	                                    Else 0 End = 1;";

                DataTable dt = (DataTable)queryManager.ReturnIQToolsObject(ClsUtility.theParams, queryStatement, ClsDBUtility.ObjectEnum.DataTable, ConnectionMode.REPORT);
